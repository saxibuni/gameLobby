                        ...
                        using (Stream entryStream = this.GetInputStream(this[entryIndex]))
                        {
                            ...
                            while ((bytesRead = entryStream.Read(buffer, 0, buffer.Length)) > 0)
                            {
                                ...
                                if (resultHandler != null)
                                {
                                    ...
                                    status.SetBytesTested(contentLength);
                                    resultHandler(status, null);
                                }
                            }
                        }
			...
			long bytesToCopy, bool updateCrc)
		{
			...
			do {
				...
				if ( bytesRead > 0 ) {
					...
					bytesToCopy -= bytesRead;
					contentLengthRead += bytesRead;
				}
			}
			while ( (bytesRead > 0) && (bytesToCopy > 0) );

			if ( contentLengthRead != targetBytes ) {
				throw new ZipException(string.Format("Failed to copy bytes expected {0} read {1}", targetBytes, contentLengthRead));
			}
