$(function() {
    ...
    //充值显示判断
    if(globalObj.iCashierUrl!= undefined && globalObj.iCashierUrl != ""){
   ...
   /* "NOEMPTY":"表单不允许有空项",
    ...
    $(".gameContainer").on('click', '.gamePic', function(event) {
        ...
        ipcRenderer.send('game-window-open', url + "|" + code + "|" + type);
    });
    ...
    var setVersionOnUI = function() {
        var website = globalObj.website;
        var version = website.substring(website.lastIndexOf("/")+1);
    ...
    var getGameInfo = function() {
        $.ajaxSettings.async = false;
        $.getJSON("json/gameInfo.json", {}, function(json) {
            gameInfo.url = json.url;
        });
        $.ajaxSettings.async = true;
