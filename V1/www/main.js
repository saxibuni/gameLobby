...
app.on('ready', () => {
    ...
    ipcMain.on('download-binery-file', (event, arg) => {
        ...
        if (process.platform == 'darwin') {
            url = downloadUrl + bineryPath + "/pc/mac/" + lanDicMap[lan] + "/" + bineryPath.toLowerCase() + "_" + lanDicMap[lan].toLowerCase() + "_" + bineryVersion.replace(/\./g, '_') + ".dmg";
        } else {
            url = downloadUrl + bineryPath + "/pc/win/" + lanDicMap[lan] + "/" + bineryPath.toLowerCase() + "_" + lanDicMap[lan].toLowerCase() + "_" + bineryVersion.replace(/\./g, '_') + ".exe";
        }
        c.exec("start " + url);
    });
